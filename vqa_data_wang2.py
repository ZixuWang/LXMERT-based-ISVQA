...
class VQATorchDataset(Dataset):
    def __init__(self, dataset: VQADataset):
        ...
        #1. to have the paths of .npy files
        feat_filenames = []
        ...
        for feat_info_path in  feat_info_paths:
            # Only kept the data with loaded image features
            if os.path.exists(feat_info_path):
    ...
    #     #合并feat和info
        for feat_key, feat_item in self.imgid2img.items():
            for info_key, info_item in self.infoid2info.items():
                if feat_key == info_key:
                    self.imgid2img[feat_key].append(self.infoid2info[info_key])
        ...
        for datum in self.raw_dataset.data: # 遍历annotation中的数据
            if datum['image_id'] in self.imgid2img: #如果anno中的image_id在.npy中，则将anno中的数据存入self.data中
                self.data.append(datum) #重新整理dataset到self.data # self.data contains refined .json data 
